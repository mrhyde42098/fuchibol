...
export async function resolveStreamForChannel(channelId: string): Promise<ResolvedStream> {
  ...
  const effectiveId = channel?.id ?? channelId;

  if (!channel && !channelId.startsWith('premium-v2-') && !channelId.startsWith('tvtvhd-') && !channelId.includes('pelotalibre')) {
  ...
  try {
    const manifestUrl = await resolveUpstreamM3u8(effectiveId);
    ...
    logger.warn({ err, channelId }, 'Stream resolution failed');
    throw new AppError(
      'STREAM_UNAVAILABLE',
      err instanceof Error ? err.message : 'No se pudo resolver el manifiesto HLS',
      502
    );
  }
}
